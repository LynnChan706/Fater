...
class Bobcat(Camera):
    ...
    def __take_picture(self):
        if self.__mark_as_open:
            ...
            logging.getLogger('bobcat_camera').info('camera info:%s' % self.get_camera_description())
    ...
    def take_picture(self, index=None):
        try:
            ...
            logging.getLogger('bobcat_camera').exception('error in take_picture:%s' % traceback.print_exc())
            img = None
        return img
